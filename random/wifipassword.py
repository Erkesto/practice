...
data = sb.check_output(['netmesh' , 'wlan' , 'show' , 'profiles']).decode('utf-8').split('\n')
...
# passwords iif they are available using the 2nd cmd command 
for i in profiles:
    #running the 2nd cmd command to check passwords
    results = sb.check_output(['netmesh' , 'wlan' , 'show' , 'profiles' , i , 'key=clear' ]).decode('utf-8').split('\n')

    #storing passwords after converting them to list 
    results = [b.split(":")[1][1:-1] for b in results if "Key Content" in b]

    #printing the profiles(wifi name) with their passwords suing try and except method
    try:
